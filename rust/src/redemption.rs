//! CTF winning-position redemption via Polymarket's builder relayer.
//!
//! Flow (gasless — relayer pays POL):
//!   1. Encode CTF.redeemPositions(USDC, 0x0, conditionId, [1, 2])
//!   2. Wrap in proxy((uint8,address,uint256,bytes)[])
//!   3. Derive proxy wallet via CREATE2 from EOA
//!   4. GET /relay-payload → (nonce, relay_address)
//!   5. Build struct hash, sign with EIP-191 personal_sign
//!   6. POST /submit (HMAC-authed) with signed relayed tx
//!   7. Poll /transaction until STATE_MINED / STATE_CONFIRMED
...
/// keccak256 of the proxy wallet's init bytecode (Gnosis-safe proxy).
...
// ── HMAC signing (matches py_builder_signing_sdk.build_hmac_signature) ─────

/// Produces a URL-safe base64 HMAC-SHA256 of (timestamp + method + path + body?),
/// keyed on the URL-safe base64-decoded secret. Matches Python's behaviour
/// exactly (including the "replace single quotes with double quotes" quirk
/// which we sidestep by building the body as JSON with double quotes natively).
fn hmac_sign(
    secret_b64: &str,
    timestamp: &str,
    method: &str,
    ...
    // Python's urlsafe_b64encode is padded. Use URL_SAFE (with padding) so
    // the signature header is byte-identical to the Python SDK's output.
...
// ── Proxy wallet derivation (CREATE2) ──────────────────────────────────────

/// Derives the Polymarket proxy wallet address for a given EOA:
///   proxy = last20(keccak(0xff ++ factory ++ keccak(eoa_bytes) ++ init_hash))
...
// ── Struct hash for the relayed tx (pre-EIP-191) ───────────────────────────
...
pub async fn redeem_positions(
    ...
    // Preserve the EXACT string Polymarket returned — they may HMAC-verify
    // against their original casing; round-tripping through `Address::parse`
    // would re-apply EIP-55 and could flip letter cases.
    ...
    // `sign_hash_sync` returns a 65-byte recoverable signature (r,s,v). We
    // want EIP-191 personal_sign — alloy's `sign_message_sync` prepends the
    // prefix before hashing. We already have the hash, so we just need the
    // prefix hash behaviour: re-hash with the Ethereum signed-message
    // prefix, then sign.
    ...
    // We computed sig_raw above only to validate the code path. Drop it to
    // avoid warnings.
    let _ = sig_raw;

    // 5. POST /submit. Body is JSON with exact key order (matches Python dict
    // insertion order) — HMAC is verified against str(body).replace("'",'"')
    // which, with double-quoted JSON, equals the wire body itself here.
    let body_str = format!(
        concat!(
